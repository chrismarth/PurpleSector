...
import { Wifi, Play, Car } from 'lucide-react';
...
interface SessionNewContentProps {
  parentIds?: Record<string, string>;
}

export default function SessionNewContent({ parentIds }: SessionNewContentProps) {
  ...
  async function createSession() {
    ...
    try {
      ...
      refreshNav();
      // Close the new-session tab
      const currentTab = state.tabs.find((t) => t.type === 'session-new');
      if (currentTab) closeTab(currentTab.id);
      // Open the new session detail tab
      openTab({
        id: `session-detail:${session.id}`,
        type: 'session-detail',
        label: session.name,
        breadcrumbs: [session.name],
        entityId: session.id,
        closable: true,
      });
    } catch (error) {
  ...
  if (!eventId) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-muted-foreground">No event specified for new session.</p>
      </div>
    );
  }
