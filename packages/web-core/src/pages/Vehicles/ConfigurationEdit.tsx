...
export default function VehicleConfigurationEditPage() {
  const { id: vehicleId, configId } = usePage().props as unknown as { id: string; configId: string };
  ...
  const configurationQuery = useQuery({
    ...
    queryFn: async () => fetchJson<any>(`/api/vehicles/${vehicleId}/configurations/${configId}`, { unauthorized: { kind: 'redirect_to_login' } }),
  ...
  const updateConfigurationMutation = useMutation({
    mutationFn: async () => {
      ...
      return mutationJson<any>(`/api/vehicles/${vehicleId}/configurations/${configId}`, { method: 'PATCH', body: { ...formData, parts: partsObject } });
    },
    onSuccess: (updated) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    try { await updateConfigurationMutation.mutateAsync(); router.visit(`/vehicle/${vehicleId}/configuration/${configId}`); }
    catch (error) { console.error('Error updating configuration:', error); alert('Failed to update configuration'); }
  }
        ...
        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            <div className="flex items-center gap-4">
              <Link href={`/vehicle/${vehicleId}/configuration/${configId}`}><Button variant="ghost" size="icon"><ArrowLeft className="h-5 w-5" /></Button></Link>
              <div>
                <CardTitle>Edit Configuration</CardTitle>
                <CardDescription>Update the parts installed on this vehicle</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <Link href={`/vehicle/${vehicleId}/configuration/${configId}`}><Button type="button" variant="outline">Cancel</Button></Link>
              </div>
            </form>
          </CardContent>
        </Card>
