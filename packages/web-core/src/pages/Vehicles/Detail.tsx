...
interface VehicleConfiguration {
  id: string; name: string; description: string | null; parts: string; createdAt: string;
  _count: { setups: number; sessions: number; };
}
interface VehicleSetup {
  id: string; name: string; description: string | null; parameters: string;
  vehicleConfiguration: VehicleConfiguration | null; createdAt: string;
  _count: { sessions: number; };
}
interface Vehicle {
  id: string; name: string; description: string | null; inServiceDate: string | null;
  outOfServiceDate: string | null; tags: string | null; createdAt: string;
  configurations: VehicleConfiguration[]; setups: VehicleSetup[];
  _count: { sessions: number; };
}
...
export default function VehicleDetailPage() {
  ...
  const deleteConfigurationMutation = useMutation({
    mutationFn: async (configId: string) => {
      await mutationJson(`/api/vehicles/${vehicleId}/configurations/${configId}`, { method: 'DELETE' });
      return configId;
    },
    onSuccess: (configId) => {
  ...
  async function deleteConfiguration(configId: string) {
    ...
    try { await deleteConfigurationMutation.mutateAsync(configId); }
  ...
  async function deleteSetup(setupId: string) {
    ...
    try { await deleteSetupMutation.mutateAsync(setupId); }
        ...
        <Card className="mb-6">
          ...
          {(vehicle.description || (vehicle.tags && JSON.parse(vehicle.tags).length > 0)) && (
            <CardContent className="space-y-3">
              {vehicle.description && <p className="text-muted-foreground">{vehicle.description}</p>}
              {vehicle.tags && (() => {
        ...
        <Tabs defaultValue="configurations" className="w-full">
          <TabsList className="grid w-full max-w-md grid-cols-2">
            ...
            <TabsTrigger value="setups" className="gap-2">
              <Wrench className="h-4 w-4" />Setups
            </TabsTrigger>
          </TabsList>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Link href={`/vehicle/${vehicleId}/configuration/new`}>
                    <Button className="gap-2"><Plus className="h-5 w-5" />New Configuration</Button>
                  </Link>
                </div>
              </CardHeader>
              <CardContent>
                {vehicle.configurations.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <Link href={`/vehicle/${vehicleId}/configuration/new`}>
                      <Button className="gap-2"><Plus className="h-5 w-5" />Create First Configuration</Button>
                    </Link>
                  </div>
                ) : (
                        ...
                        <CardContent>
                          <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-2 text-sm">
                              <div><span className="text-muted-foreground">Setups:</span><span className="ml-2 font-semibold">{config._count.setups}</span></div>
                              <div><span className="text-muted-foreground">Sessions:</span><span className="ml-2 font-semibold">{config._count.sessions}</span></div>
                            </div>
                            <div className="flex gap-2">
                              ...
                              <Button variant="outline" size="sm" onClick={() => deleteConfiguration(config.id)}>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Link href={`/vehicle/${vehicleId}/setup/new`}>
                    <Button className="gap-2"><Plus className="h-5 w-5" />New Setup</Button>
                  </Link>
                </div>
              </CardHeader>
              <CardContent>
                {vehicle.setups.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <Link href={`/vehicle/${vehicleId}/setup/new`}>
                      <Button className="gap-2"><Plus className="h-5 w-5" />Create First Setup</Button>
                    </Link>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {vehicle.setups.map((setup) => (
                      <Card key={setup.id} className="hover:shadow-lg transition-shadow">
                        <CardHeader>
                          ...
                          {setup.description && <CardDescription>{setup.description}</CardDescription>}
                        </CardHeader>
                        <CardContent>
                          <div className="space-y-3">
                            {setup.vehicleConfiguration && (
                              <div className="text-sm">
                                ...
                                <Badge variant="outline" className="ml-2">{setup.vehicleConfiguration.name}</Badge>
                              </div>
                            )}
                            <div className="text-sm">
                            ...
                            <div className="flex gap-2">
                              ...
                              <Button variant="outline" size="sm" onClick={() => deleteSetup(setup.id)}>
                                <Trash2 className="h-3 w-3" />
                              </Button>
                            </div>
                          </div>
