...
import { MathTelemetryChannel } from '@purplesector/web-telemetry';
...
function ColumnResizeHandle({
  ...
  return (
    <div
      {...bind()}
      className="absolute top-0 right-0 w-2 h-full cursor-col-resize hover:bg-primary/20"
...
interface AnalysisPanelGridProps {
  ...
  /** Whether the parent TelemetryDataPanel is in fullscreen mode. */
...
export function AnalysisPanelGrid({
  ...
  // Compute gridTemplateRows: use explicit pixel height for drag-resized rows, 1fr for others
  const gridTemplateRows = useMemo(() => {
    const numRows = Math.max(...layout.panels.map((p) => p.y + p.rowSpan), 1);
    const rowSizes: string[] = [];
    for (let r = 0; r < numRows; r++) {
      // Find a panel starting at this row that has been explicitly resized (minHeight === maxHeight)
      const resizedPanel = layout.panels.find(
        (p) => p.y === r && p.rowSpan === 1 && p.minHeight != null && p.maxHeight != null && p.minHeight === p.maxHeight,
      );
      if (resizedPanel) {
        rowSizes.push(`${resizedPanel.minHeight}px`);
      } else {
        rowSizes.push('1fr');
      }
    }
  ...
  return (
    <TelemetryHoverContext.Provider value={{ hoverIndex, setHoverIndex }}>
      <div
        ref={containerRef}
        className={focusPanelId ? 'w-full h-full' : 'gap-2 h-full'}
        style={
          focusPanelId
            ? { display: 'flex' }
            : {
...
function PanelCell({
  ...
  return (
    <div
      className="relative rounded-md border bg-card flex flex-col"
      style={
        focusPanelId
    ...
    >
      ...
      <div
        className={`px-3 pt-2 pb-1 min-w-0 flex-1 min-h-0 overflow-hidden flex flex-col`}
      >
