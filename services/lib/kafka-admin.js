 ...
 * Kafka Admin Utility
 * 
 * Handles Kafka administrative tasks:
...
const logger = require('./logger').child({ component: 'kafka-admin' });
...
class KafkaAdmin {
    ...
    try {
      ...
      if (!existingTopics.includes(config.kafka.topics.telemetry)) {
        topicsToCreate.push({
          ...
          numPartitions: 10, // 10 partitions for parallel processing
          replicationFactor: 1, // Increase to 3 in production
          configEntries: [
            { name: 'compression.type', value: 'producer' }, // Use producer's compression setting
            { name: 'retention.ms', value: '3600000' }, // 1 hour
            { name: 'segment.ms', value: '600000' }, // 10 minutes
            { name: 'min.insync.replicas', value: '1' }, // Increase to 2 in production
          ],
        });
      }
  ...
  /**
   * Ensure a user-specific topic exists
   */
